...
void sendGET() //pvoutput
{
  ...
  if (client.connect(servername, 80)) {  //Send to pvoutput
    if (sw == 0) {
      ...
      Serial.println(Directory + "?key=" + MyKey + "&sid=" + MySID + "&d=" + String(year()) + months + days + "&t=" + hours + ":" + mins + "&v3=" + EU + "&v4=" + PU);
    }
    ...
    client.println(Directory + "?key=" + MyKey + "&sid=" + MySID + "&d=" + String(year()) + months + days + "&t=" + hours + ":" + mins + "&v3=" + EU + "&v4=" + PU);
...
void meterupdate() {
  ...
  today = day();   // Reset KWH at end of day.
  if (lastDay != today) {
    lastDay = today;
    pulseCount = 1;
  }
...
void setup() {  //Meter Pulse.
  // KWH interrupt attached to SCL/D1 = GPIO 5
...
void onPulse()
{
  ...
  if ((pulseTime - lastTime) > 180000) {  //180mS filter for max power of 20kw reading = 3600000000/20000 = 180000uS = 180mS
    pulse = 1;
    pulseCount++;  // Count of the number of pulses since start of day
  }
  else {
    pulseTime = lastTime;
    lastTime = savedTime;
